...
HF_TOKEN="${HF_TOKEN:-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}"
...
docker_build_and_push() {
  ...
  if docker buildx version >/dev/null 2>&1; then
    docker buildx build \
      ...
      --build-arg PRELOAD_ASR_MODEL="$PRELOAD_ASR_MODEL" \
      --build-arg ASR_MODEL_ID="$ASR_MODEL_ID" \
      --build-arg ASR_MODEL_REVISION="$ASR_MODEL_REVISION" \
...
main() {
  ...
  echo "Building stt-worker image for platform ${IMAGE_PLATFORM}..."
